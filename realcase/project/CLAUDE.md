...
sync it back, commit.** (The 2026-08-19 day on MUSICA is history: `MIGRATION_MUSICA.md`,
`HANDOVER_MUSICA_TO_VSC5.md`; nothing from it applies here except the data layout.)
...
0. **`HANDOVER_2026-08-20.md`** — current state; the 2026-08-21 update at the top
   carries the six-run result. **Start here.**
1. `DECISIONS.md` — the two 2026-08-21 entries and the 2026-08-20 ones below them.
2. `branko/OPEN_ISSUES.md` **A10** (slope-factor energy pairing, measured),
   **A11** (bootstrap trap — real, not the lever), **A12** (morning runaway).
3. `branko/KNOWN_ISSUES.md` **U2, E11–E16** — the traps that cost time.
...
- **The turbulence deficit against the MYNN control is the closure's
  stable-regime equilibrium, not spin-up or bootstrap**: q² in the lowest ~100 m
  equilibrates by 02:30 at one third of MYNN; the ratio is 0.96 in neutral and
  unstable air and a flat 0.22 above Ri ≈ 0.3, where 69 % of the nocturnal valley
  cells sit. **MYNN is the control, not the truth** — it is known to over-mix at
  night and under-forecast the valley wind. **Rule: no change to constants, the
...
- **Slope-factor energy pairing**: ~90 % of the horizontal-pairing shear
  production (a third of all production, essentially all on slopes) is never
  paid by the resolved flow. Fixed in minimal form behind `pbl3d_sf_pair=1`
  (default 0); acceptance test = run X6, the in-model residual
  `KE_LOSS_H + QSQ_SHEAR_H/2` must fall from ~0.9 to ~0 of the production.
- **Morning runaway (A12)**: every run, the unchanged code included, blows up
  2–2.5 h after sunrise at ridge-top columns in neutral, strongly sheared
  plunging flow. Diagnosis run: restart from 04:00 with the 1-minute budget
  (`branko_runs/innval_pbl3d_A12`, output `exp/A12`). Until it is contained, no
  run is expected to pass 06:00 UTC.
- **The strain cap (`pbl3d_sk_eps_max = 6`) is load-bearing**: 12 or off brings
  the nocturnal runaway back within 45 min.
- The floor on the asymptotic length (`pbl3d_l0_min`), the equilibrium start
  (`pbl3d_init_opt`) and the Ri-aware cap (`pbl3d_limiter_opt=2`) exist as
  default-off options; the first two change q² by 2–4 %, the third is unrun.
...
- Loosen `pbl3d_sk_eps_max` above 6, or tune the stable regime before the
  observation comparison, or tune anything before the pairing fix is in.
- Run more than one experiment with the same `WRF_OUTPUT_ROOT` (they clobber
  each other live in `temp/branko/`) — one env file per run,
  `realcase/env/vsc5_X<n>.sh` → `exp/X<n>`.
...
- MYNN control: job 8320565, `wrf_output/8320565/`, 01:00 start, 30-min frames.
  Reference 3D runs: 8476273 (1 h, bit-identical to X0), `exp/X0..X5/wrf_output/`.
...
- Toolchain: Spack gcc 12.2.0 + OpenMPI 4.1 via `realcase/env/vsc5.sh` (`module
  ...
  versions end up on `PATH`; 4.1.4 wins; left alone deliberately.
