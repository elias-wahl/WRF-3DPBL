...
dropped --- and if dropped, what is assumed instead. Table~I.1b (added 2026-09-16) traces the nineteen rows back to the hypotheses of turbulence theory they come from --- scale separation, the Reynolds conditions, ergodicity, Taylor's hypothesis, homogeneity, stationarity, isotropy in three forms, locality, similarity, and the constraints of realizability and Galilean invariance --- shows which observation of the campaign tests each, and lists five assumptions the rows do not yet carry (candidates D1--D5); Figure~I.1b draws the chain of reliance as a pyramid. It comes in three versions: a one-page
...
\pG\ \pL & the row is this hypothesis of turbulence theory written out for the closure \,/\, the row holds only if the hypothesis also holds --- Table~I.1b clusters the rows by the hypotheses behind them \\
\end{tabularx}
\end{small}
