...
# homogeneity is drawn on top of similarity: cut its band out of similarity's top edge
hom = next(b for b in blocks if b["title"].startswith("Horizontal"))
sim = next(b for b in blocks if b["title"] == "Similarity")
band = [t for t in hom["tops"] if t[0] > 13][0]
lo, hi = min(t[0] for t in sim["tops"]), max(t[1] for t in sim["tops"])
bx0 = min(t[0] for t in hom["tops"] if t[0] > 13)
bx1 = max(t[1] for t in hom["tops"] if t[0] > 13)
sim["tops"] = [(lo, bx0), (bx1, hi)]
hom["tops"] = [t for t in hom["tops"] if t[0] < 13] + [(bx0, bx1)]
...
ROWS = [
    ...
    ("Monin--Obukhov",
     ...
     S + r": Stationarity, Similarity and, through the channel, Horizontal homogeneity. " + L + r" (text): Constant-flux layer. " + C + r": Surface fluxes and first level; Master length scale (left part: the wall branch $\kappa z$ with its stability correction).",
