...
\newcommand{\hecto}[2]{}
\hecto{Locality}{By day eddies span $z_i$: the flux stops answering the local gradient; on slopes it arrives from the slope--valley interface. Holds at night.}
...
\hecto{Local equilibrium}{Resolved strain by day rotates faster than $l/q$; transitions and intermittency at night are not slow against it: the strain cap is the admission.}
...
\hecto{Mixing length}{$z_i$ ill-posed under elevated inversions, wall distance ambiguous on a slope; the horizontal eddy set by the valley width: two lengths at least.}
...
\hecto{Eddy viscosity}{The grid filter (aspect ratio 25) imprints its anisotropy on the subgrid eddies; the stress axes leave the gradient where slope wind meets valley wind.}
