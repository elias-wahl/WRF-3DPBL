...
\pb{1.93}{5.40}{3.6}{6.1}{row}{\textbf{Grid means are ensemble means}\\ the grid spacing appears nowhere, every eddy inside the cell, $\Delta \gg l,\ z_i$; the resolved flow is not turbulent, so its shear feeds production; the vertical grid resolves the shear layer it differentiates, $\Delta z \ll$ its depth}
\pbf{5.68}{6.85}{3.6}{6.1}{row}{\textbf{Vertical shear only}\\ two of nine terms; no gradient of $W$}{0.44}
\pbf{7.05}{9.17}{3.6}{6.1}{row}{\textbf{Vertical divergence only}\\ means and TKE in $z$ only; TKE not advected; horizontal mixing by numerics at its own scale, its energy lost, not paid to $\qsq$; surfaces horizontal}{0.36}
...
\pbf{11.15}{13.05}{3.6}{6.1}{row}{\textbf{Level-2.5 truncation}\\ all moments but $\qsq$ algebraic; $S_M$, $S_H$ from Rotta's balance $P_s + P_b = \varepsilon$ in the vertical Ri; cutoff at $\Rfc$}{0.44}
...
\pbf{14.90}{17.20}{3.6}{6.1}{row}{\textbf{Master length scale}\\ $1/L = 1/L_S + 1/L_T + 1/L_B$: wall distance with $\zeta$, one $z_i$ per column, $q/N$ from the buoyancy term; the same $L$ in $K$ and $\varepsilon$; blind to terrain and grid}{0.44}
\pb{17.40}{18.55}{3.6}{6.1}{row}{\textbf{Constant set}\\ one set, fitted in neutral and convective air over flat terrain}
\pb{18.75}{20.42}{3.6}{6.1}{row}{\textbf{Dissipation}\\ $\varepsilon = q^3/(B_1 L)$: isotropic, one rate, diagnosed from the master length}
\pb{20.60}{21.75}{3.6}{6.1}{row}{\textbf{Pressure terms}\\ Rotta return, $A_1$; isotropised production, $C_1$; TKE transport down-gradient, $S_q$}
...
\node[inner sep=0pt] at (17.975,6.45) {\color{cMYNN}\gK};
\node[inner sep=0pt] at (17.975,6.80) {\color{cGXIX}\gK};
\node[inner sep=0pt] at (17.975,7.15) {\color{cAPPROX}\gK};
\node[inner sep=0pt] at (17.975,7.50) {\color{cFULL}\gK};
\node[inner sep=0pt] at (19.585,6.45) {\color{cMYNN}\gK};
\node[inner sep=0pt] at (19.585,6.80) {\color{cGXIX}\gK};
\node[inner sep=0pt] at (19.585,7.15) {\color{cAPPROX}\gK};
\node[inner sep=0pt] at (19.585,7.50) {\color{cFULL}\gK};
\node[inner sep=0pt] at (21.175,6.45) {\color{cMYNN}\gK};
\node[inner sep=0pt] at (21.175,6.80) {\color{cGXIX}\gK};
\node[inner sep=0pt] at (21.175,7.15) {\color{cAPPROX}\gK};
\node[inner sep=0pt] at (21.175,7.50) {\color{cFULL}\gK};
...
B6, split into the constant-flux layer and Monin--Obukhov similarity. Wall value of $\qsq$: the lower boundary of the TKE equation, part of B6. Surface fluxes and first level: D3 with the bulk-transfer half of B6. Local fluxes: C1. K-form fluxes: C2, C3 and the mixing of all scalars alike. Master length scale: C4 with the one-length statement of C5. Dissipation: C5. Level-2.5
