...
\begin{frame}{What we changed since forking the scheme --- the essentials}
  The scheme, as forked, did not survive real mountains. Five changes made
  ...
  \begin{enumerate}\small
    ...
    \item \textbf{Exact budget bookkeeping built in} (WRFlux, extended to the
      turbulence energy): every tendency of heat, momentum and TKE decomposed
      online into named processes --- the instrument behind the energy audit
      and the cold-pool findings.
  \end{enumerate}
\end{frame}
...
\begin{frame}{Result 2 --- the full-day scorecard, 18 July: one day, every metric}
  Soundings and lidar (from 100\,m), mixed-layer depth per launch --- same
  day, same grid, same forcing; \textbf{bold = better}.
  \vspace{1ex}
  \begin{center}
  ...
  \end{center}
  \vspace{1ex}
  Eight of nine metrics favour the 3D closure in both layers --- the daytime
  temperature bias by a factor of 4--7 --- with the one exception (wind
  direction) within a few degrees.
\end{frame}
