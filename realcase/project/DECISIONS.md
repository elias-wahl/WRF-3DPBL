...
**2026-09-01, ~00:30 — X10 stays halted at segment d (Elias): the morning state comes first. Rationale: X10's unique results (midday start does not fix the sunrise pool; night gate; wind compression) are extracted; its daytime would inherit the known +2 K pool bias; and any outcome of the cold-air investigation forces a re-run of the full day anyway. The chain stays resumable (`scontrol release 8551754`); cancel only if the configuration definitely changes. Critical path: CPB1 pool budget → mechanism → soundness-checked proposal.**
