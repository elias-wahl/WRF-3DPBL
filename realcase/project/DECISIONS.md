...
**2026-08-30, ~12:30 — A19 attack launched: forcing from the ICON run initialized 2025-07-17 12 UTC (Elias's decision), WRF start 17 UTC-evening-before so the model builds its own cold pool; old ICON forcing deleted for disk (Elias's authorization). Pipeline rebuilt non-interactively.**
...
**2026-08-30, ~10:30 — θ inside the BL: the fix's +1.2 K at 11 UTC is inherited from 08 UTC, not generated; the control's +0.4 K is two errors cancelling. Retraction: the warm offset is *not* a uniform column offset — the free atmosphere (2–3 km) is unbiased; the offset is BL-confined and predates the fix.**
...
**2026-08-30, ~09:45 — 11–12 UTC verdict: with the wall open the Inn-valley boundary layer reaches the observed depth to within ~200 m and one hour — the valley lag was A18's wall, not a transport deficit; S_q retired; dark-night gate `N3S` submitted (the fix also removes essentially all Tier-3 clipping, which a sunlit `N2S` cannot certify for the stable night).**
