...
*Next.* The A17 fix candidates go to Elias for the soundness review before any code; the S_q production pairs are deferred until A17 is decided — testing transport above a zero wall flux is the wrong order. Tools: `run_split_segment.sh` (any window/split/lane, chained), `chain_segment.slurm` takes `MINUTES`. Lanes at 12:00: only `zen3_2048` reaches us (E-note in memory `vsc5-queues`), next nodes 16:16 / 17:17 / 02:24.
