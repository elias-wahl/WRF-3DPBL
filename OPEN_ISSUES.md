...
3. **p_sfc is extrapolated linearly in height from the two lowest mass levels** (`start_em.F:2731–2737` form: z from φ(1..3), p from levels 1 and 2). p(1) is finite because t1 = θ(p/p₀)^κ printed finite ⇒ **the NaN was born in p(2) or φ(3) — the first refined layer**, which at this crest is ~8.5 m thick (the design's minimum; 13.5 m at 80 levels).
