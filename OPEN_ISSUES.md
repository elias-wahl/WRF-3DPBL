...
## A39 — OPEN, BUG (2026-09-26): `pbl3d_scalar_mix = 1` is a no-op under `pbl3d_opt = 2` — condensate and number concentrations are never mixed by the closure
Only `Diagnose_fluxes_pbl_approx` (reached for `pbl3d_opt < 2`) fills `turb_flux_*q{c,r,i,s,g,n*}`; the `pbl3d_opt = 2` branch calls `Calc_fluxes`, whose interface has no hydrometeor gradients/fluxes (`module_pbl3d_my.F:297–306`, `:1185–1192`); the fluxes stay at their allocation value 0. Verified: 12-min restart smoke with the switch on is bit-for-bit identical to the control (332 variables). Side slip: `module_first_rk_step_part2.F:1345–1349` passes the u-flux in the w slot of `scalar_bc_pbl3d` (ghost rows only, harmless). Minimal fix (agent H): with `pbl3d_scalar_mix = 1` compute w-fluxes −min(khz, 0.4/(dt·rdz²))·∂X/∂z per species behind the existing switch. Expected effect on the morning layer small (closure fluxes ≤ 4 % of vertical advection there). DECISIONS 2026-09-26 14:59.

## A38 — OPEN, BUG (2026-09-26): with `use_theta_m = 1` the 3D closure's θ and qv tendencies bypass WRF's θ → θm conversion — its moistening cools the air (≈ 0.19·LE at the surface)
**Refined 15:34 2026-09-26 (the 15:20 'surface only' wording is withdrawn):** two parts — surface boundary condition (missing 1.608·θ·E/ρ; `wthv` dry) AND interior: θᵥ built from the perturbation θm − 300 but converted back with the full θm (`Calc_heat_flux`, `module_pbl3d_my.F:2738–2777`) → applied θm flux short by 0.608·T₀·w′qᵥ′. Column sink 1.608·θ·E ≈ 0.19·LE (≈ 62 % in layer 1, ≈ 38 % where the subgrid moisture flux converges). Size (HERO2, ∫ from 05 UT, spread over 1–1.5 km): 0.3–0.8 K by 13 UT day 1, 0.6–1.3 K by 14 UT day 2 (upper bound). The fix warms the daytime boundary layer (valley already 1–3 K too warm at 11 UT day 2); pair it with A36. Not a first-order cause of the cloud errors (MYNN twins share them). DECISIONS 2026-09-26 15:20, 15:34.
`conv_t_tendf_to_moist` (`module_first_rk_step_part2.F:1053`) runs before the closure's tendencies are added (`:1182–1442`); the closure evolves `t_2` = θm − 300 as if it were θ (`module_pbl3d.F:168`) and applies the surface heat flux HFX/(ρcp) without the (Rv/Rd)·θ·QFX/ρ term (`module_pbl3d_my.F:3297`; WRF's diffusion has it, `module_diffusion_em.F:4484–4487`). Consequence: a spurious heat sink cp·θ·(Rv/Rd)·E ≈ 0.19·LE at the surface (40–54 W m⁻² at midday in HERO2, 14–18 % of HFX) and cooling wherever the closure's moisture flux converges (BL top, cumulus base). Every 3D-closure run so far uses `use_theta_m = 1`. Test running: `use_theta_m = 0` restart twin LE3M vs LE2C. Fix candidate: feed the closure dry θ and convert its tendency with the same formula as `conv_t_tendf_to_moist`, behind a default-off switch. DECISIONS 2026-09-26 14:59.

## A37 — OPEN, BUG (2026-09-26): the 3D closure's PBLH is the terrain height + ≈ 17 m — `Diagnose_pbl_height` tests ASL heights (`grid%z_at_w`) against a 200 m AGL bound
`dyn_em/module_pbl3d_my.F` ≈ 4115 (`do while (z_at_w(i,k,j) .le. 200.)`); z_at_w comes from `module_first_rk_step_part2.F:1254` as `grid%z_at_w` (ASL). Above 200 m terrain the lowest-200 m search never runs and zi returns the first layer top in ASL: HERO2 PBLH − HGT = 15–18 m everywhere (03 and 12 UT), range 200–3702 m. Used by `Calc_scale_aware` (dxdh = 2.5Δx/min(PBLH,3000); `pbl3d_scale_aware = 1` in HERO2): the Honnert taper sees terrain height, not BL depth (≈ 0.92 on crests instead of ≈ 1.0 at night). Explains E45. Fix: use z_at_w − ht (or AGL wall heights) behind a default-off switch; check every other user of `pblh` in the closure (e.g. `pbl3d_lh_opt`). DECISIONS 2026-09-26 13:37.
**Second consumer (15:34 2026-09-26):** sfclayrev's convective velocity w* = (g/Tₛ·PBLH·F_b)^⅓ (`sf_sfclayrev.F90:369–386`) → u* over land → the closure's surface stress −u*² and surface q². HERO2 noon: wind factor S_bug/S_true 0.90–0.92 on the floor, 1.03–1.04 lee, 1.06–1.13 crests → stress ≈ −15…−19 % / +6…+8 % / +12…+28 % (first-order, daytime only). Noah-MP's H/LE are not affected. DECISIONS 2026-09-26 15:34.

## A36 — FIXED OPT-IN 12:51 2026-09-26 (`noah_mp:noahmp_tv_conv = 1`, branch 3dpbl_noahmp_tvconv; default 0 bit for bit; residual now equals the canopy storage; production decision pending) — BUG (2026-09-26): Noah-MP v5's canopy energy budget is not conserved at WRF's 2 s time step — +36…+38 W m⁻² of spurious surface flux over vegetation at noon (≈ 75 % into H), ≈ 10 W m⁻² at night
`phys/noahmp/src/SurfaceEnergyFluxVegetatedMod.F90:218–331`: the canopy-temperature iteration exits on |δTV| ≤ 0.01 K (`:329`) while the per-step heat-capacity term FVEG·HCV/Δt (≈ 8000 W m⁻² K⁻¹ at Δt = 2 s) dwarfs the flux derivatives (≈ 250 W m⁻² K⁻¹); the first iteration of each step is neutral (`ResistanceAboveCanopyMostMod.F90:87–89`); the unconverged remainder is booked as canopy heat storage, so `BalanceErrorCheckMod.F90` passes. Measured in HERO2 at 11 UT: FSA − FIRA − HFX − LH − GRDFLX = −36 (crop), −38 (needleleaf), 0.0 (barren); offline HRLDAS at 900 s: 0. Toy of the loop: −33/−9/−2/0 W m⁻² at 2/18/60/900 s. Test: HRLDAS offline at NOAH_TIMESTEP = 2 s. Fix candidates (default-off switch): converge on the energy residual, or start each step from the previous step's stability. Report upstream (NCAR/noahmp, wrf-model/WRF) once reproduced offline. DECISIONS 2026-09-26 11:35.
...
Lee-slope v (0–300 m AGL) departs from ICON's 00 UT cycle by −1.1 m s⁻¹ at 10 UT, −2.6 at 13, −4.2 at 17; the lee–floor buoyancy barrier falls 1.7 → 0.3 K between 09 and 12 UT (ICON 2.0 → 1.4) because the floor warms 1 K faster than ICON's (lee θ within ±0.25 K), while the floor takes +60…+90 W m⁻² more sensible heat than ICON's surface (A30, larger in HERO2's land set); Innsbruck turns northerly at 12 UT. 2–6 Δx band at 1800 m: 0.23 (10 UT) → 0.31–0.41 (11–15 UT) vs ICON 0.21 and X12mta 0.20–0.37. The 13–14 UT onset in the ledger was set by the 13 UT start of the lineages. **Persistence (09:25):** one continuous event (floor footprint 0.39 at 12 UT, 0.28 at 13, 0.66 at 17–17:30, 0.10 at 20 UT; 13 UT lineages 0.35 at 16, 0.63, 0.07 at 20:30); same peak in every run; HERO2 adds a nocturnal crest northerly at 01–03 UT (−2.0…−2.5 m s⁻¹) that stays off the floor; after withdrawal the up-valley wind restarts in the Hall–Jenbach reach (3.2–3.9 m s⁻¹, 18:30–20 UT) and outlives the observed one by 2 h at Kolsass. No same-state control: the floor heat, the UCM towns, filter option 5 and the start time are not separated. DECISIONS 2026-09-26 09:10.
...
**2026-09-10 ~14:30.** Raw ICON (no cloud fields downloaded): the layer is ICON's at 19–20 UT (15 % of reach columns saturated, WRF 17 %) and dissipated by 22 UT (2 %) while WRF keeps 24 % saturated — inherited at 19:30–20:00, WRF's own persistence at 21:30–23:00. The 18th evening is cloud-free in model and radiometer.

**To do.** (1) FTP inventory for ICON cloud fields (CLC/QC/CLCT) before the next download. (2) A 19:00-restart twin with the forcing's humidity above 2 km capped at 90 % (or the ladder's humidity spliced in) — judge on the reach cooling 21:30–23:00, the Hall–Jenbach pressure force and the Kolsass wind. (3) The 18th evening: same check.
...
**Inferred from the code (`dyn_em/module_pbl3d_my.F:492–559`; to be confirmed from arrays):** option 2 lengthens the master scale in three places relative to production option 1 —
1. asymptotic scale l₀ = α₁ × (q-weighted turbulence height) with the hard-coded 3DTKE value **α₁ = 0.8**, against α = 0.1 of `pbl3d_constants='MY82'` (`:4184`): **8× longer at the same q²**;
2. unstable air: l_b = l_f = 10¹⁰ m (`:555`) — **the buoyancy cap l ≤ N_TAU_MAX q/N (0.53 q/N) that option 1 applies (`:520–524`) is bypassed**; the only bound left is the strain cap inside the solve, l ≤ 2·SK_EPS_MAX/b₁ · q/|S| = 0.72 q/|S| (`:1726`);
3. stable air: l_b = α₂ q/N (1 + α₃/α₂ √(q_c/(l₀N))) with α₂ = 1.0 — ≥ 1.9× the option-1 cap; the surface scale gains κz(1 − 100 z/L)^0.2 in unstable air.
The closure's tendencies are applied forward-Euler (`dyn_em/module_pbl3d.F:525–603`: `u_tmp += tend·dt/μ`). Its own vertical mixing therefore obeys the explicit-diffusion limit **K Δt/Δz² < ½**: with Δt = 2 s and Δz ≈ 16 m at a crest, K > 64 m² s⁻¹ is unstable; K ≈ S_M l q reaches 90 m² s⁻¹ with l = 300 m, q = 3 m s⁻¹, S_M = 0.1 — ordinary numbers for a heated crest at noon once l₀ is 8× longer and unbounded by buoyancy. The instability of explicit diffusion is a growing 2Δz oscillation of the diffused field — the 31.5 m/s level-1 wind is what it looks like at the ground.

**Link to A24 (hypothesis):** X13 thinned Δz from 13.5 to 8.5 m at the same cell class (barren crest, `znt 0.002`, beside a `znt 1.09` forest cell where u* > 1) — the same criterion tightened 2.5× at unchanged K; it died 3 h 50 min in, at the hottest hour, when K was largest. X14 raised K at unchanged Δz and died in 14 min. The two twins that kept the production l and Δz both live. If the arrays confirm it, A24 and A25 are one defect — **the closure's vertical diffusion is applied explicitly, and any change that lengthens l or thins Δz near the ground finds the limit first at convective barren crests** — and the remedy is numerical (an implicit vertical solve for the closure tendencies, or a diffusion-number cap on l), not a constant.

**What settles it (staged, `exp/x14_diag/STAGED_COMMANDS.sh`, python denied in the auto-session):** `run_diag.sh` → `x14smoke_crash_cell.log`: at both cells and A24's cell, from the 13:00 and 13:10 frames and the twin's 13:00/13:30: L_MASTER, Q_SQ, the diffusion-number estimate 0.1·l·q·Δt/Δz² at k = 1–8, the k = 1–8 wind (a 2Δz oscillation at 13:10 = the explicit instability already under way), θ, W and Courant, tier diagnostics, 3 × 3 stencils, and the domain maxima of |V|₁, UST, Q_SQ, L_MASTER, COND_A. The 13:00 L_MASTER ratio to the twin is the option's pure effect at an identical state. For A24, `run_x13r.sh` adds the same diffusion number to the 1-min record.

**Not to do:** re-run `pbl3d_l_opt=2` as is; retune α₁, α₂ or SK_EPS_MAX; read this as a verdict on the MYNN length scale itself (MYNN applies it inside an implicit column solver with its own α = 0.23 and bounded l_b — the option here is the blend with 3DTKE's constants in an explicit 3D solver). **Consequence for X14's question** (carrying-capacity deficit, morning growth): unanswered; the experiment needs a variant of the option that keeps the production α and the buoyancy cap (a new default-off switch and a rebuild, E16) — or the numerical remedy first.
...
**Inferred from the code** (`phys/physics_mmm/sf_sfclayrev.F90`; to be confirmed from arrays):
1. **The NaN came in through p_sfc, not through the level-1 state and not through Noah-MP's q_sfc.** hfx = C_p ρ_sfc u* θ* with ρ_sfc = p_sfc/(R T_v) (`:334`), θ_g = T_sk (p₀/p_sfc)^κ (`:264`), θ* = κ(θ₁−θ_g)/ψ_h (`:826`); u* = ½ u*_old + ½ κ|V|/ψ_m (`:809`) and C_h = u* κ/ψ_q (`:945`) never touch p_sfc. A NaN q_sfc reaches θ_v,g and qfx (`:362`, `:911`) but not θ* (`dtg = thx − thgb`, `:541`) — hfx would have stayed finite and the detector (ust/hfx/chs) silent.
2. **`br = 0, zol = 0` exactly is the fingerprint of a NaN Ri.** `if(mol<0) br = amin1(br,0)` (`:390`) — gfortran's MIN/MAX return the non-NaN argument — so after an unstable step a NaN bulk Richardson number leaves as 0.0, the scheme takes the forced-convection branch (`:483`, ψ = 0, zol = 0) and u*, C_h come out finite. With finite T_sk, T₁, q₁, |V| that is only possible if p_sfc (or its parents) is NaN.
3. **p_sfc is extrapolated linearly in height from the two lowest mass levels** — at run time `p8w(i,1,j) = w1·p(1) + w2·p(2)` in `phy_prep` (`module_big_step_utilities_em.F:5122`, weights from z at w-levels 1–3), copied into the surface field every step by the surface driver (`module_surface_driver.F:2048`, `PSFC(I,J)=p8w(I,kts,J)`); the same form as the initialisation (`start_em.F:2731–2737`). p(1) is finite because t1 = θ(p/p₀)^κ printed finite ⇒ **the NaN was born in p(2) or φ(3) — the first refined layer**, which at this crest is ~8.5 m thick (the design's minimum; 13.5 m at 80 levels).
4. **The cell**: land, `znt 0.002 m` = Noah-MP's snow-surface roughness (`NoahmpTable.TBL:447 Z0SNO`) ⇒ snow/ice-covered (USGS category 24 or thin snow); first mass level ≈ 7 m AGL from κ|V|/u* = 8.13 = ln((z+z₀)/z₀) ⇒ near the highest terrain (domain first-level range 7.2–9.2 m, HGT max 3621 m); 14 m/s at 7 m with a 4.5 K superadiabatic skin–air step.

**Candidates (undecided):** (a) vertical Courant / w-damping failure in the 8.5 m layer under afternoon convection over a heated crest — the risk the smoke was meant to gate, fallback Δt = 1.5 s (+33 %); (b) the level-2.5 closure closing on an 8.5 m layer with Δu/Δz of order 1 s⁻¹ (S k/ε cap, 10×10 solve, q² diffusion — cf. A23's horizontal-diffusion form ∝ q²·q); (c) the pairing of a 7 m surface-layer height with Noah-MP snow physics is unchanged from the twin and is not a candidate on its own.

**What settles it:** the staged reproduction **X13r** (`exp/x13_diag/STAGED_COMMANDS.sh`): restart from 16:00 on the same 5 × 128 layout (E14), 1-min stream 23 from 16:35 with `realcase/iofields_x13r.txt` (PSFC, QSFC, MOL, BR, REGIME, HFX, QFX, W, T, P, PH, Q_SQ, tier flags); first the cheap scan `exp/x13_diag/x13a_crash_cell.py` (cell + twin at 16:00/16:30, domain NaN count, max vertical Courant and where). If W at k = 2–3 or the Courant number climbs over the last frames → (a); if `PBL3D_T3_FLAGS`/`COND_A` or Q_SQ spike at the cell first → (b). Extending the detector to print p_sfc, q_sfc, qfx, θ* and θ/p/w(k=2) costs a 12-min relink (E16: no pending job may link `wrf.exe`).
...
1. **Horizontal q² diffusion (`pbl3d_opt = 2` only, `Calc_q_sq_horizontal_diffusion`, `module_pbl3d.F:6101`).** The flux at `:6190–6192` carries an extra factor ½(q²_{k+1}+q²_k) in the slot where the WRF template (`module_diffusion_em.F:3958`, `xkxavg = ... 0.5*(rho+rho)`) has the density — the effective coefficient is S_q·l·q·q², not a diffusivity; the metric term `- dq_sq_dz_dz_dx` sits outside the `-msfux*xkxavg*(...)` bracket where the template has it inside (`module_diffusion_em.F:3990–3992`), so the terrain correction is multiplied neither by K nor by the map factor; the divergence (`:6291–6300`) carries an overall factor 2 inherited from WRF's `doing_tke` doubling, so K_q^h = 2·S_q·l·q against K_q^v = S_q·l·q; and S_q here is a hard-coded `Sq = 0.20` (`:6125`), not `pbl3d_sq`. Active in every production run (X6–X10 lineage). Test: a 12-min devel run with the term disabled versus enabled, `QSQ` difference field; if the q² factor is real, the term scales with q³ and is loudest in the convective mixed layer.
2. **θ_v from the perturbation θ (`Calc_thetav`, `module_pbl3d.F:1693`).** `thetav = th_tmp*(1 + 0.608*qv/(1+qv))` with `th_tmp` = `grid%t_2` = θ − 300 (with `use_theta_m = 1`, θ_m − 300), so the interior θ_v misses 300·0.608·q_v ≈ 1.8 K at q_v = 10 g/kg while the surface face is filled with θ_v,sk − 300 from the full θ (`:5488–5491`, `:5594–5597`). N², the buoyancy limit 0.53 q/N and the level-2 Ri inherit the difference; the vertical gradient of the missing term is 300·0.608·∂q_v/∂z, i.e. the moisture contribution to N² is off by a factor ~θ/(θ−300) in sign-preserving but wrong magnitude. Test: print θ_v at one column against θ(1+0.608 q_v) from the same arrays.

Also recorded from the same reading, for the Goger ports (not this tree): `goger19WRF/phys/module_bl_mynn_wrapper.F` computes `defor12_destaggered` (`:368–376`) but passes the raw corner-staggered `defor12` (`:579`); the G18 port passes the destaggered array (commit "Destaggered def12 for the calculation of qHSP"). In both ports `pdk(kts) = pdk1 - pdk(kts+1)` (`module_bl_mynn.F:3335` in G18) cancels the horizontal shear production in the lowest layer, and the mass-point HSP is inserted into an interface budget. Candidate for the "suspected defect" of the G19 port named in the concept.

## A21 — OPEN: the pressure-level forcing cannot carry the valley WIND either — the 13:00 IC hands WRF a featureless 0.5-2.5 m/s ramp over 0-800 m AGL where the lidar shows a formed 6 m/s valley jet (A19's momentum twin, 2026-09-02)
Measured with the dlid86 VAD (Kolsass, 17 July): IC band mean 2.2 m/s vs observed 5.8 at 13 UT (~40 %), and no jet shape at all — the 584-1522 m ASL gap interpolates linearly across the entire jet layer. Causal chain through the first evening: WRF spins a circulation up from nothing (peak 17 UT, three hours late, hollow above 300 m), it dies early (18:30-19:30), the arriving drainage re-stirs the layer through the observed deep calm (model 2.4-3.4 vs observed 0.2 m/s at 21 UT, 50-150 m) — the collapse interregnum never happens and the pool forms warm. EXONERATION half (18 July, X10e vs lidar): with a model-generated history the valley wind is right-class (8.3-9.5 vs 10.9-11.3 m/s at peak, ~20 % low) — the deficit is the forcing chain, not the closure/dynamics. Consequences: (1) the first afternoon+evening of every cold-started run is unusable for circulation/collapse physics (01:00 starts have the nocturnal analog); (2) fix candidates: ICON model-level output for metgrid (availability = ACINN question) or starts early enough for in-model spin-up (~06 UT for an afternoon target); (3) X10f (18th evening) is the pending collapse test with a healthy circulation. Figures: plot_output/diagnostics/kolsass17_timeheight.png, kolsass17_profiles.png; DECISIONS 2026-09-02 ~16:30/~17:00.

**2026-09-03 — root and fix:** the gap was our own converter keeping ICON's 11 isobaric levels while the raw files carry 65 model levels (DECISIONS 2026-09-02 ~20:30). Fixed in icon2wrf `--ml-plevs` (36-level ladder, jet preserved at 3.8 m/s at Kolsass); tested by the **X12 chain** (DECISIONS 2026-09-03 ~10:50). **Scope correction from X10f:** on the 18th, with a spun-up circulation, the wind decay and the 100–300 m cooling are right to 1–3 m/s / 0.8 K, but the 0–100 m formation deficit recurs (+2.0 K at 21 UT) — A21 explains the wind and the layer above 100 m, not the near-surface deficit.

## A20 — OPEN, mechanism measured: the explicit 6th-order hyperdiffusion (`diff_6th_opt = 2`, factor 0.12) warms the nocturnal Inn-valley cold pool at +0.12…+0.15 K/h (0–300 m AGL) by diffusing along terrain-following surfaces from the warmer sidewall air; over a 10–11 h night ≈ +1.3–1.7 K — the size of the sunrise deficit (2026-09-01)
Measured directly from `T_TEND_DAMP_MEAN` (pure filter here: the θ-Rayleigh branch requires `damp_opt = 2`, we run 3 — KNOWN_ISSUES E35) in the two 10-min pool-budget probes (01 and 04 UT, `exp/CPB1d`/`exp/CPB2d`), with the full θ budget closed by the exact WRFlux flux divergences (`wrf3dpbl-diag/pool_adv_decomp.py`; closure gap ≤ 0.08 K/h). At 0–50 m the filter delivers +0.238 → +0.272 K/h (01 → 04 UT), growing as the pool sharpens, while resolved advective warming decays +0.298 → +0.088 and the deep pool's advective term is a net cooling (−0.12). The implicit upwind diffusion of the 5th/3rd-order advection scheme is negligible (+0.02…+0.04 K/h; `pool_upwind_diffusion.py`). Structure: strongest in the lowest 25 m, concentrated within ~1 km of the sidewalls (nearly the whole 2–3 km-wide floor): the 7-point stencil connects pool-floor cells to wall cells whose η level sits 100–300 m higher and 1–3 K warmer — downgradient along η, so the monotonic option does not stop it; the slope taper (thresh 0.10) leaves floor-to-wall pairs below threshold untapered. Shared verbatim by the MYNN control → explains the closure-independence of the pool bias. Refines A19: the forcing gap heals by 05 UT (X10), and this is the engine of the model's own too-warm nocturnal equilibrium. Candidate responses: (1) `diff_6th_opt = 0` — **DONE 2026-09-01, verdict: NOT the lever — the resolved flow re-supplies ~85–90 % of the removed warming at equilibrium (pool 0.04–0.09 K colder after 4 h; DECISIONS ~21:00); A20 downgraded to numerics hygiene**: the pool-budget twins 8552759/8552760 were repurposed in place (namelist edit only, queue position kept; self-cancelling pair → one run, 01→05 UT, full WRFlux instrumentation on); judge by the budget's filter row (must be exactly 0), the pool cooling rate (prediction: −0.16 → ≈ −0.3 K/h at 0–300 m by 04 UT), the 05 UT pool against the Kolsass sonde, and the 2Δx noise of w/θ against `exp/X10c`. (2) `diff_6th_thresh` reduction — **downgraded to partial**: the taper tests only the two cells adjacent to each flux interface (`|phb(i)−phb(i−1)|` vs `thresh·g·dx`) while the stencil spans ±3 cells, so flat floor–floor interfaces near the walls leak wall-θ into the pool at any threshold (DECISIONS 2026-09-01 ~17:00); usable as a production compromise, not as the clean experiment. (3) a z-coordinate-aware (truly horizontal) filter would be a source change.
