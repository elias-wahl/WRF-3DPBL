...
## A32 — OPEN (2026-09-25): every model-vs-HATPRO comparison and four judge scripts are mis-sited — Kolsass column against an instrument that stood at Radfeld (E72) — and use a θ convention 0.75 K too warm
**Measured** (DECISIONS 2026-09-25 ~10:00, `exp/x16_judge/hatpro_site_check.log`): at the correct site the night bias level stands (Radfeld 0–100 m +3.2…+3.9 K 00–05 UT; Kolsass 0–300 m +2.3…+2.4 K by the 05 UT sonde), but before sunset the model is 0.3–1.0 K too cold at Radfeld (recorded +1.0…+1.5 warm), it cools only 26–58 % of the observed 0–100 m cooling over 21–01 UT, and the 21 UT ICON start is already +1.75 K warm in the lowest 100 m.
**To do.** (1) Re-point to a Radfeld E59 box with the TAWES-pressure θ: `hatpro_cooling.py` (constants `LAT, LON, ELEV`, p0), `evening_arrival_check.py` §θ, `hatpro_vs_sondes.py` (not a validation any more — relabel as an along-valley comparison), `x16_theta_budget.py` (obs column), `icon_init_check.py`; judges `branko/realcase/scripts/hero_judge.slurm` §4, `judge_x12.slurm`, `budget_x16.slurm`, `wrf3dpbl-diag/judge_evening.slurm` (hard-coded 0–300 m HATPRO trajectory in its header text). (2) Redo the 09-17 transition θ budget at a Radfeld box against HATPRO. (3) For the Kolsass evening, the only thermal profiles are the 18 July sondes — any Kolsass-specific evening statement needs them or the i-Box towers. Footprint of (1)/(2): diagnostics only, no model change.

## A31 — OPEN (2026-09-25): odd–even (2Δt / 2Δz) instability of the closure's explicit vertical scalar transport in a strongly turbulent nocturnal crest layer — killed HERO at 03:50 UT
