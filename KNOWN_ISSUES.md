...
## E75 — advection on constant-AGL surfaces must use the terrain-relative vertical velocity: −u·∇ₛθ − (w − u·∇h) ∂θ/∂z; pairing the along-surface ADV_H with the Cartesian −w ∂θ/∂z counts slope-following flow as descent (2026-09-26)
`lee_barrier_physical.py` did exactly that until 2026-09-26: on the evening lee slope the spurious −(u·∇h)∂θ/∂z was +20…+28 K/h and the "budget" summed to +22…+29 K/h against an actual −0.3 K/h; it produced the retracted evening mechanism (DECISIONS 2026-09-26 09:30). The identity is exact (s = z − h, h time-independent), not an approximation. **Rule.** Any budget on terrain-following or AGL surfaces: either take every derivative on true constant-height (ASL) surfaces, or use w − u·∇h with the along-surface gradients — and always print the actual tendency next to the sum of terms; a sum that misses the tendency by more than the terms left out is a method failure, not physics.

## E76 — the Kolsass (UIBK) and Radfeld (MeteoSwiss) HATPRO absolute humidity in the lowest 100 m is 1.5–3 g/m³ below the co-located in-situ sensors (2026-09-26)
17 July: Kolsass 0–100 m 7.4–9.2 g/m³ vs the i-Box tower's ≈ 10.6–10.8 g/m³ (from T/RH/p); Radfeld 9.0–9.1 vs its own MET sensor ≈ 10.9 g/m³ at 09–11 UT. Radiometer humidity profiles have coarse vertical resolution and a retrieval prior; use them for column integrals and time changes, never for near-surface absolute humidity. In-situ 2 m (TAWES, i-Box) and the sondes are the references (DECISIONS 2026-09-26, moisture trace).

## E77 — ICON surface-series cycle before 09 UT; natural-tile diagnostics in town cells; forest T2/Q2 height (2026-09-26)
(a) `ICON/surface_series/icon_surface_2025040100_2025071800.nc` is stitched from forecast cycles: before 09 UT on 17 July it is the 16 July 12 UT cycle, not the 00 UT cycle that forces HERO/HERO2 (floor LE 20–37 % lower at 07–08 UT). Compare a run against its own cycle (native GRIBs, `agentB_icon_native.npz`), never against the series before 09 UT. (b) With `sf_urban_physics = 1` and Noah-MP, TR, ETRAN, EDIR, EVG in the history are natural-tile values, not scaled by (1 − FRC_URB); the grid LH/HFX are the blended values. (c) Over forest Noah-MP's T2/Q2 sit ≈ 15 m above ground (above the displacement height) — not comparable with a 2 m thermometer.

## E78 — Noah-MP runs in single precision in WRF: never book an energy term as C·(T − T_start) when C is large (2026-09-26)
`kind_noahmp = 4`; a temperature near 300 K resolves ≈ 3·10⁻⁵ K, and the per-step canopy heat capacity at Δt = 2 s is ≈ 8000 W m⁻² K⁻¹, so a differenced storage carries ≈ 0.2 W m⁻² of round-off — enough to trip Noah-MP's 0.01 W m⁻² balance check (the first A36 fix stopped all 640 ranks in step 1). Accumulate increments instead. Also: land-surface questions can be answered by 12-min restart smokes on the devel QOS (5 × 128 fits in 6:40 with history every 6 min) — the land surface responds within minutes, the air hardly at all (DECISIONS 2026-09-26, A36).

## E80 — the Stanser Joch sonic (i-Box sf23) faces SOUTH: the station tables' aspect 340° is wrong (2026-09-26)
The 10 m Tirol DEM (`data/geo/DGM_Tirol_10m_epsg31254_2006_2020.tif`) puts sf23 (47.39504 N 11.68714 E) at exactly 1861 m on a 22° slope facing 156° (SSE), with the ridge 150–280 m higher within 300–600 m to its north: a lee-slope site on the sunlit side of the northern range, not a north-facing crest site. `data/stations/ibox_ecpy/stations.csv` and `proc/config/ibox_stations.csv` list aspect 340°. Its wind cycle agrees with the DEM (downslope from the north at night, upslope from the SSE by day). Any model-cell selection that matched sf23 to north-facing cells sampled the wrong side of the ridge; match it to south-facing cells within ~1.3 km (model slope there ≈ 11° vs 22° real). DECISIONS 2026-09-26 20:38.
...
**Cause corrected 20:43 2026-09-26 (verified):** the restart state was consistent; the boundary file is not — `real.exe` writes θm into `wrfbdy` when `use_theta_m = 1` (`module_initialize_real.F:4925–4938`), and a `use_theta_m = 0` run relaxes toward it as if it were dry θ: LE3S outer 10 rows +0.99 K vs LE0 (interior −0.01 K), MU +360 Pa gained through the boundaries. To test `use_theta_m = 0` from a restart, regenerate `wrfbdy` with `use_theta_m = 0`. DECISIONS 2026-09-26 20:43.
