...
## E70 — a WRF restart is NOT a faithful re-entry in the HERO configuration (2026-09-25): identical at the restart time, 74 % of cells differ one minute later (T2 up to 0.47 K), and the closure's odd–even scalar mode comes back 3 × stronger
Measured with HR0 against HDIAG (DECISIONS 2026-09-25 02:55). The closure's state is in the restart file, so something else is lost or re-initialised; which field is being tested (HR0B). Until it is known, a restart twin cannot reproduce a marginal event, and every segment boundary of a chained run shocks the closure.
